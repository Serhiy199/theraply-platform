    ...
    <SurfaceCard as="section">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-base leading-7 text-slate-600">
            This directory gives the operations team a clean view of client accounts,
            activation state, and account creation flow across the platform.
          </p>
        </div>
        <InsetCard as="div" tone="plain" className="px-4 py-3 shadow-none">
          <span className="font-semibold text-slate-900">{users.length}</span> client
      ...
      {users.length ? (
        ...
        <DashboardEmptyState
          ...
          description="Client registrations will appear here once account activity starts flowing into the platform."
        />
      )}
    </SurfaceCard>
  );
