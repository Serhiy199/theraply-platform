...
| `src/app/therapist/onboarding/actions.ts#uploadTherapistCertificatesAction` | Server action | `assertActionRole(user, [THERAPIST])`; service locks upload to editable onboarding states. | File count, MIME type, extension, and 10MB size validation. Draft data is saved if valid. | Missing AuditLog for certificate upload. | Missing | Critical upload endpoint; add rate limit and optional malware/content scanning later. |
...
| `src/server/services/certificate-storage.service.ts` | Upload provider abstraction, MIME/extension/size validation, editable-state guard, metadata-only DB storage. | Add AuditLog for upload success/failure and rate limit upload action. |
